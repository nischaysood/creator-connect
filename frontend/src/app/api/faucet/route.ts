...
export async function POST(req: Request) {
    try {
        ...
        const mneeBalance = await client.readContract({
        ...
        // Mint MNEE if low
        if (mneeBalance < parseEther('100')) {
            console.log(`[Faucet] Minting 10,000 MNEE to ${address}...`);
            await client.writeContract({
                address: MOCK_MNEE_ADDRESS as `0x${string}`,
                abi: MOCK_MNEE_ABI,
                functionName: 'mint',
                args: [address as `0x${string}`, parseEther('10000')]
            });
            funded = true;
        }
        ...
        return NextResponse.json({
            ...
            balances: {
                ...
                mnee: formatEther(newMneeBalance)
            }
        });
